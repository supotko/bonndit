...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef void mw_openmp_multc(double[:,:] x, double[:,:] signal, double[:,:] est_signal, double[:,:] dipy_v, double[:,:,:,:] pysh_v, double[:,:,:,:] rot_pysh_v, double[:,:] angles_v, double[:,:,:] dj, double[:] loss, int amount, int lmax, int num_of_dir) nogil:
        minimize_watson_mult(&x[0,0],&signal[0,0],&est_signal[0,0],&dipy_v[0,0],&pysh_v[0,0,0,0],&rot_pysh_v[0,0,0,0],&angles_v[0,0],&dj[0,0,0],&loss[0],amount,lmax,num_of_dir)

@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef void mw_openmp_singlec(double[:] x, double[:] signal, double[:] est_signal, double[:] dipy_v, double[:,:,:] pysh_v, double[:,:,:] rot_pysh_v, double[:] angles_v, double[:,:,:] dj, double[:] loss, int lmax, int num_of_dir) nogil:
        minimize_watson_mult(&x[0],&signal[0],&est_signal[0],&dipy_v[0],&pysh_v[0,0,0],&rot_pysh_v[0,0,0],&angles_v[0],&dj[0,0,0],&loss[0],1,lmax,num_of_dir)
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef void mw_openmp_single_o4c(double[:] x, double[:] signal, double[:] est_signal, double[:] dipy_v, double[:,:,:] pysh_v, double[:,:,:] rot_pysh_v, double[:] angles_v, double[:] loss, int num_of_dir) nogil:
        minimize_watson_single_o4(&x[0],&signal[0],&est_signal[0],&dipy_v[0],&pysh_v[0,0,0],&rot_pysh_v[0,0,0],&angles_v[0],&loss[0],num_of_dir)
